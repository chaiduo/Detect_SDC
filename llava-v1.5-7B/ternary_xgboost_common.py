...
def add_significant_sdc_target(df):
    ...
    significance = pd.to_numeric(df["significance"], errors="coerce")
    if label.isna().any():
    ...
    if values.issubset({0, 1}):
        binary = ((label == 1) & (significance.astype(int) == 2)).astype(int)
    else:
...
def run_ternary_xgboost(
    ...
    summary = {
        ...
        "class_names": CLASS_NAMES,
        "target_definition": {
            "0": "non-significant samples: original label == 0 or non-significant original label == 1",
            "1": "significant_sdc: original label == 2, or original label == 1 and significance == 2",
        },
