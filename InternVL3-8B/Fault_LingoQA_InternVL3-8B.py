...
def evaluate_on_vqa(model_path, val_file, data_dir, output_jsonl, device):
    ...
    idx = 0
    results = []
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Faulting"):
        question_id = row["question_id"]
        segment_id = row["segment_id"]
        gt_answer = row["answer"]

        for i in range(5):
            injector.reset_and_select_target()
            ...
            before_score, after_score, clear_pred = score_evaluator.get_fault_scores(question, gt_answer, prediction, idx)
            ...
            if abs(after_score - before_score) > 0.2:
...
def main():
    ...
    output_jsonl = "./json/Fault_LingoQA-InternVL3-8B.jsonl"
