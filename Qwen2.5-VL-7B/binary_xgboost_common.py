...
def as_binary_series(series, column_name):
    values = pd.to_numeric(series, errors="coerce").fillna(0).astype(int)
    bad_values = sorted(set(values.unique().tolist()) - {0, 1})
    if bad_values:
        raise ValueError(f"{column_name} 只支持 0/1，检测到: {bad_values}")
    return values
...
def add_targets(df):
    ...
    if "significance" not in df.columns:
        raise ValueError("CSV 缺少 significance 列")

    label = as_binary_series(df["label"], "label")
    significance = pd.to_numeric(df["significance"], errors="coerce").fillna(-1).astype(int)

    df = df.copy()
    df["sdc_target"] = label
    df["significant_sdc_target"] = ((label == 1) & (significance == TARGET_SIGNIFICANCE)).astype(int)
    return df
...
def split_train_test(df, group_col):
    if group_col not in df.columns:
        raise ValueError(f"CSV 缺少分组列 {group_col}")
    ...
    if overlap:
        raise AssertionError(f"train/test 的 {group_col} 有重叠，存在泄漏风险: {len(overlap)}")
...
def run_binary_xgboost(
    ...
    tasks = {
        "sdc": {
            "target_col": "sdc_target",
            "description": "predict label == 1",
        },
        "significant_sdc": {
            "target_col": "significant_sdc_target",
            "description": "predict label == 1 and significance == 2",
        },
    }
