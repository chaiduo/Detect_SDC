...
# 配置区域
# =========================
TRAIN_CSV = "./train_data/Qwen2.5_LingoQA_train_set.csv"
VALID_CSV = "./train_data/Qwen2.5_LingoQA_valid_set.csv"
...
# # 配置区域
# # =========================
# CSV_PATH = "sdc_features.csv"
# GROUP_COL = "sample_uid"
# FIGURE_DIR = "./figure"

# ORIG_ID_THRESHOLD = 4000
# TRAIN_ORIG_ID_COUNT = 4000
# RANDOM_STATE = 42

# # 切分方式:
# # 1) "group_shuffle"
# # 2) "orig_id_threshold"
# # 3) "orig_id_random_train_n"
# SPLIT_MODE = "orig_id_threshold"

# # Optuna 配置
# N_TRIALS = 50
# # OPTUNA_DIRECTION = "maximize"   # maximize AUC
# # OPTUNA_METRIC = "auc"           # 当前目标固定为 AUC

# OPTUNA_DIRECTION = "minimize"   # maximize AUC
# OPTUNA_METRIC = "logloss"           # 当前目标固定为 AUC
# # =========================
# # 工具函数
# # =========================
# def ensure_dir(path):
...
# def print_metrics(y_true, y_pred, y_prob=None, title="Metrics"):
#     acc = accuracy_score(y_true, y_pred)
#     precision = precision_score(y_true, y_pred, zero_division=0)
#     recall = recall_score(y_true, y_pred, zero_division=0)
#     f1 = f1_score(y_true, y_pred, zero_division=0)
...
# def plot_confusion_matrix_fig(cm, save_path="confusion_matrix.png"):
...
#         plt.figure(figsize=(8, 6))
#         plt.plot(train_logloss, label="Train logloss", linewidth=2)
#         plt.plot(val_logloss, label="Val logloss", linewidth=2)
#         plt.xlabel("Boosting Round")
#         plt.ylabel("Logloss")
#         plt.title("Training Curve - Logloss")
#         plt.legend()
...
#         plt.figure(figsize=(8, 6))
#         plt.plot(train_auc, label="Train AUC", linewidth=2)
#         plt.plot(val_auc, label="Val AUC", linewidth=2)
#         plt.xlabel("Boosting Round")
#         plt.ylabel("AUC")
#         plt.title("Training Curve - AUC")
#         plt.legend()
...
#     elif split_mode == "orig_id_threshold":
#         if "orig_id" not in df.columns:
#             raise ValueError("列 orig_id 不存在，无法按 orig_id 阈值切分")

#         orig_id = pd.to_numeric(df["orig_id"], errors="coerce")
#         if orig_id.isna().any():
#             bad_count = int(orig_id.isna().sum())
#             raise ValueError(f"orig_id 中有 {bad_count} 个值无法转为数值，不能进行阈值切分")

#         train_mask = orig_id < orig_id_threshold
#         val_mask = orig_id >= orig_id_threshold

#         if train_mask.sum() == 0:
#             raise ValueError("按 orig_id 阈值切分后，训练集为空")
#         if val_mask.sum() == 0:
#             raise ValueError("按 orig_id 阈值切分后，验证集为空")
...
#         split_info = {
#             "split_mode": split_mode,
#             "orig_id_threshold": orig_id_threshold,
#             "train_rule": f"orig_id < {orig_id_threshold}",
#             "val_rule": f"orig_id >= {orig_id_threshold}"
#         }
#         return train_idx, val_idx, split_info

#     elif split_mode == "orig_id_random_train_n":
#         if "orig_id" not in df.columns:
#             raise ValueError("列 orig_id 不存在，无法按 orig_id 随机分组切分")

#         orig_id = pd.to_numeric(df["orig_id"], errors="coerce")
#         if orig_id.isna().any():
#             bad_count = int(orig_id.isna().sum())
#             raise ValueError(f"orig_id 中有 {bad_count} 个值无法转为数值，不能进行随机分组切分")

#         unique_orig_ids = np.array(sorted(orig_id.unique()))
#         total_unique = len(unique_orig_ids)

#         if train_orig_id_count <= 0:
#             raise ValueError("train_orig_id_count 必须大于 0")
#         if train_orig_id_count >= total_unique:
#             raise ValueError(
#                 f"train_orig_id_count={train_orig_id_count} 不能大于等于唯一 orig_id 数量 {total_unique}"
#             )

#         rng = np.random.RandomState(random_state)
#         shuffled_orig_ids = unique_orig_ids.copy()
#         rng.shuffle(shuffled_orig_ids)

#         train_orig_ids = set(shuffled_orig_ids[:train_orig_id_count])
#         val_orig_ids = set(shuffled_orig_ids[train_orig_id_count:])

#         train_mask = orig_id.isin(train_orig_ids)
#         val_mask = orig_id.isin(val_orig_ids)

#         if train_mask.sum() == 0:
#             raise ValueError("随机选取 orig_id 后，训练集为空")
#         if val_mask.sum() == 0:
#             raise ValueError("随机选取 orig_id 后，验证集为空")
...
#         split_info = {
#             "split_mode": split_mode,
#             "train_orig_id_count": int(train_orig_id_count),
#             "total_unique_orig_id": int(total_unique),
#             "val_orig_id_count": int(total_unique - train_orig_id_count),
#             "random_state": random_state
#         }
#         return train_idx, val_idx, split_info
...
# def build_model(params, scale_pos_weight):
...
#     if "num_steps_in_group" in df.columns:
#         exclude_cols.append("num_steps_in_group")
...
#         print(f"Group column: {GROUP_COL}")
#     elif SPLIT_MODE == "orig_id_threshold":
#         print(f"orig_id threshold: {ORIG_ID_THRESHOLD}")
#     elif SPLIT_MODE == "orig_id_random_train_n":
#         print(f"Train unique orig_id count: {TRAIN_ORIG_ID_COUNT}")
...
#     print(f"Total NaN in features: {total_nan}")
#     print("=" * 60)

#     # -------------------------
#     # 切分
#     # -------------------------
#     train_idx, val_idx, split_info = split_data(
#         df=df,
#         X=X,
#         y=y,
#         split_mode=SPLIT_MODE,
#         group_col=GROUP_COL,
#         orig_id_threshold=ORIG_ID_THRESHOLD,
#         train_orig_id_count=TRAIN_ORIG_ID_COUNT,
#         random_state=RANDOM_STATE
#     )
...
#     if SPLIT_MODE == "group_shuffle":
#         train_groups = df.iloc[train_idx][GROUP_COL]
#         val_groups = df.iloc[val_idx][GROUP_COL]
#         print(f"  Train unique groups: {train_groups.nunique()}")
#         print(f"  Val unique groups:   {val_groups.nunique()}")

#     if "orig_id" in df.columns:
#         train_orig = df.iloc[train_idx]["orig_id"]
#         val_orig = df.iloc[val_idx]["orig_id"]
#         print(f"  Train orig_id unique: {train_orig.nunique()}")
#         print(f"  Val orig_id unique:   {val_orig.nunique()}")
...
#     print("Start Optuna hyperparameter tuning...")
#     print("=" * 60)
...
#     print(f"Best trial value ({OPTUNA_METRIC}): {study.best_value:.6f}")
#     print("Best params:")
...
#     model = build_model(best_params, scale_pos_weight)

#     print("Start final training with best params...")
#     print("=" * 60)
...
#     y_pred_default = (y_prob >= 0.5).astype(int)
#     default_metrics = print_metrics(
#         y_val,
#         y_pred_default,
#         y_prob=y_prob,
#         title="Validation Metrics @ threshold=0.50"
#     )

#     best_thr, best_f1 = find_best_threshold(y_val, y_prob, start=0.05, end=0.95, step=0.01)
#     y_pred_best = (y_prob >= best_thr).astype(int)

#     print(f"Best threshold on validation for F1: {best_thr:.2f}")
#     print(f"Best F1 found by threshold search: {best_f1:.6f}")
#     print("=" * 60)
...
#         y_pred_best,
#         y_prob=y_prob,
#         title=f"Validation Metrics @ best threshold={best_thr:.2f}"
#     )
...
#         "best_iteration": int(model.best_iteration) if hasattr(model, "best_iteration") else None,
#         "best_score": float(model.best_score) if hasattr(model, "best_score") else None,
#         "suggested_scale_pos_weight": float(scale_pos_weight),
#         "default_threshold_metrics": default_metrics,
#         "best_threshold": float(best_thr),
#         "best_threshold_metrics": best_metrics
#     }
...
#     plot_training_curves(evals_result, save_dir=FIGURE_DIR)
#     plot_confusion_matrix_fig(cm, save_path=os.path.join(FIGURE_DIR, "confusion_matrix.png"))
#     plot_feature_importance_fig(
#         feat_imp,
#         top_n=20,
#         save_path=os.path.join(FIGURE_DIR, "feature_importance_top20.png")
...
#     if len(set(y_val)) > 1:
#         plot_roc_curve(y_val, y_prob, save_path=os.path.join(FIGURE_DIR, "roc_curve.png"))
